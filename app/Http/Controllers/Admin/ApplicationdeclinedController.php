...
class ApplicationdeclinedController extends Controller
{
  const VIEW_PATH = 'admin.applicationdeclined.';
  ...
  {
    $this->authorize('read',Applicationdeclined::class);
    $applications = Applicationdeclined::with('association','area','branch','category','service', 'serviceType', 'user','file','approval')->whereIn('approval_id',[Admin::REJECTED])->orderBy('id','desc')->utwd()->paginate(100);
  ...
  {
    $this->authorize('create',Applicationdeclined::class);
  ...
  public function store(Request $request)
  {
    ...
    try {
      ...
      $application = Applicationdeclined::create($data);
    ...
    return redirect()->route('admin.applicationdeclined')->with([
  ...
  {
    $this->authorize('update',Applicationdeclined::class);
    $application = Applicationdeclined::with('association','area','branch','category','service','serviceType','user','file')->where(['id'=>$id])->first();
  ...
  public function update(Request $request, $id)
  {
    ...
    try {
      $application = Applicationdeclined::where(['id'=>$id])->first();
      if ( is_null($application) == true) {
        return back()->with([
          'error' => __('admin.common.error'),
          'alert-type' => 'error'
      ...
      Applicationdeclined::find($id)->update($data);
  ...
  {    
    $this->authorize('delete',Applicationdeclined::class);
    $application = Applicationdeclined::with('file')->find($id);
    if ( is_null($application) == true) {
      return back()->with([
        'error' => __('admin.common.error'),
        'alert-type' => 'error'
  ...
  {   
    ...
    return redirect()->route('admin.applicationdeclined.create')->with([
      'message' => __('admin.common.success'),
      'alert-type' => 'success'
    ]);
  }
  ...
  {
    $this->authorize('update_approval',Applicationdeclined::class);
    $application = Applicationdeclined::with('association','area','branch','category','service','serviceType','user','file')->where(['id'=>$id])->first();
    if ( is_null($application) == true) {
      return back()->with([
        'error' => __('admin.common.error'),
