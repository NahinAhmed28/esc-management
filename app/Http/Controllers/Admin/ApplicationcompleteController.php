...
use App\Models\Applicationcomplete;
...
class ApplicationcompleteController extends Controller
{
  const VIEW_PATH = 'admin.applicationcomplete.';
  ...
  {
    $this->authorize('read',Applicationcomplete::class);
    $applications = Applicationcomplete::with('association','area','branch','category','service', 'serviceType', 'user','file','approval')->whereIn('approval_id',[Admin::COMPLETED])->orderBy('id','desc')->utwd()->paginate(100);
  ...
  {
    $this->authorize('create',Applicationcomplete::class);
  ...
  public function store(Request $request)
  {
    ...
    try {
      ...
      $application = Applicationcomplete::create($data);
    ...
    return redirect()->route('admin.applicationcomplete')->with([
  ...
  {
    $this->authorize('update',Applicationcomplete::class);
    $application = Applicationcomplete::with('association','area','branch','category','service','serviceType','user','file')->where(['id'=>$id])->first();
  ...
  public function update(Request $request, $id)
  {
    ...
    try {
      $application = Applicationcomplete::where(['id'=>$id])->first();
      if ( is_null($application) == true) {
        return back()->with([
          'error' => __('admin.common.error'),
          'alert-type' => 'error'
      ...
      Applicationcomplete::find($id)->update($data);
  ...
  {    
    $this->authorize('delete',Applicationcomplete::class);
    $application = Applicationcomplete::with('file')->find($id);
    if ( is_null($application) == true) {
      return back()->with([
        'error' => __('admin.common.error'),
        'alert-type' => 'error'
  ...
  {   
    ...
    return redirect()->route('admin.applicationcomplete.create')->with([
      'message' => __('admin.common.success'),
      'alert-type' => 'success'
    ]);
  }
  ...
  {
    $this->authorize('update_approval',Applicationcomplete::class);
    $application = Applicationcomplete::with('association','area','branch','category','service','serviceType','user','file')->where(['id'=>$id])->first();
    if ( is_null($application) == true) {
      return back()->with([
        'error' => __('admin.common.error'),
